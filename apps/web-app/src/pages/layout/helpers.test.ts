...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("neocode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("neocode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("neocode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("neocode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("neocode://open-project")).toBeUndefined()
    expect(parseDeepLink("neocode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "neocode://open-project?directory=/a",
      "neocode://other?directory=/b",
      "neocode://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NEOCODE__: {
        deepLinks: ["neocode://open-project?directory=/a"],
      },
    } as unknown as Window & { __NEOCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["neocode://open-project?directory=/a"])
