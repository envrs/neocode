import { base64Decode } from "@neocode-ai/pkg-utils/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, withProject }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "neocode-e2e-project-nongit-"))
  ...
  try {
    await withProject(async () => {
      ...
      expect(path.basename(activeDir)).toContain("neocode-e2e-project-nongit-")
