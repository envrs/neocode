...
async function main() {
  ...
  const extensionToml = await $`git show ${version}:extensions/zed/extension.toml`.text()
  ...
  await $`git config user.email "63023139+khulnasoft-bot@users.noreply.github.com "`
  ...
  // Delete any existing branches for neocode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating NeoCode extension to v${cleanVersion}"`
