...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/neopilot-ai/neocode/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/neopilot-ai/neocode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- services/core packages/sdk packages/plugin apps/desktop apps/web-app apps/ext-vscode apps/ext-zed infrastructure/ci`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      ...
      else if (file.startsWith("extensions/vscode/")) areas.add("extensions/vscode")
      else if (file.startsWith("platform/ci/")) areas.add("github")
    }
...
async function summarizeCommit(neocode: Awaited<ReturnType<typeof createNeocode>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await neocode.client.session.create()
  const result = await neocode.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "neocode", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], neocode: Awaited<ReturnType<typeof createNeocode>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(neocode, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/neopilot-ai/neocode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const neocode = await createNeocode({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, neocode)
    notes.push(...lines)
