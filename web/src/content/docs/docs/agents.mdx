---
title: Agents
description: Configure and use specialized agents in neocode.
---
...
You can create new agents using the `neocode agent create` command. This interactive command will:
...
neocode comes with a built-in `general` agent:
...
Agents can be configured in your `neocode.json` config file or as markdown files.
...
- Global: `~/.config/neocode/agent/`
- Project: `.neocode/agent/`

```markdown title="~/.config/neocode/agent/code-reviewer.md"
---
description: Reviews code for best practices and potential issues
model: anthropic/claude-sonnet-4-20250514
tools:
...
5. **Project-specific agents** - Use `.neocode/agent/` for project-specific workflows

## Examples

### Documentation Agent

```markdown title="~/.config/neocode/agent/docs-writer.md"
---
...
```markdown title="~/.config/neocode/agent/security-auditor.md"
---
