...
import (
	...
	"github.com/neopilot-ai/neocode/internal/components/diff"
	...
	"github.com/neopilot-ai/neocode/internal/viewport"
)
...
func (m *messagesComponent) renderView() tea.Cmd {
	...
	return func() tea.Msg {
		...
		orphanedToolCalls := make([]neocode.ToolPart, 0)
		...
		for _, msg := range slices.Backward(m.app.Messages) {
			if assistant, ok := msg.Info.(neocode.AssistantMessage); ok {
		...
		for _, message := range m.app.Messages {
			...
			case neocode.UserMessage:
				...
				for partIndex, part := range message.Parts {
					...
					case neocode.TextPart:
						...
						fileParts := make([]neocode.FilePart, 0)
			...
			case neocode.AssistantMessage:
				...
				for partIndex, p := range message.Parts {
					...
					case neocode.TextPart:
						...
						remainingParts := message.Parts[partIndex+1:]
						toolCallParts := make([]neocode.ToolPart, 0)
						...
						for _, part := range remainingParts {
							...
							case neocode.ToolPart:
								...
								if part.State.Status != neocode.ToolPartStateStatusCompleted && part.State.Status != neocode.ToolPartStateStatusError {
			...
			error := ""
			if assistant, ok := message.Info.(neocode.AssistantMessage); ok {
				switch err := assistant.Error.AsUnion().(type) {
				case nil:
				case neocode.AssistantMessageErrorMessageOutputLengthError:
					error = "Message output length exceeded"
				case neocode.ProviderAuthError:
					error = err.Data.Message
				case neocode.MessageAbortedError:
...
func (m *messagesComponent) renderHeader() string {
	...
	shareEnabled := m.app.Config.Share != neocode.ConfigShareDisabled
...
func (m *messagesComponent) CopyLastMessage() (tea.Model, tea.Cmd) {
	...
	var lastTextPart *neocode.TextPart
	for _, part := range lastMessage.Parts {
		if p, ok := part.(neocode.TextPart); ok {
			lastTextPart = &p
		}
	}
	if lastTextPart == nil {
