...
func TestLoadThemesFromJSON(t *testing.T) {
	...
	expectedThemes := []string{"tokyonight", "neocode", "everforest", "ayu"}
...
func TestLoadThemesFromDirectories(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(userConfig, "neocode", "themes"), 0755)
	os.MkdirAll(filepath.Join(projectRoot, ".neocode", "themes"), 0755)
	os.MkdirAll(filepath.Join(cwd, ".neocode", "themes"), 0755)
	...
	os.WriteFile(filepath.Join(userConfig, "neocode", "themes", "override-test.json"), []byte(testTheme1), 0644)
	os.WriteFile(filepath.Join(projectRoot, ".neocode", "themes", "override-test.json"), []byte(testTheme2), 0644)
	os.WriteFile(filepath.Join(cwd, ".neocode", "themes", "override-test.json"), []byte(testTheme3), 0644)
