...
// 2. USER_CONFIG/neocode/themes/*.json
// 3. PROJECT_ROOT/.neocode/themes/*.json
// 4. CWD/.neocode/themes/*.json
func LoadThemesFromDirectories(userConfig, projectRoot, cwd string) error {
	...
	dirs := []string{
		...
		filepath.Join(projectRoot, ".neocode", "themes"),
	}
	if cwd != projectRoot {
		dirs = append(dirs, filepath.Join(cwd, ".neocode", "themes"))
	}
