...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/neocode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/neocode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/neocode/")) areas.add("core")
