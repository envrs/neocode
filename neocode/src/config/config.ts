...
export namespace Config {
  ...
  export const state = App.state("config", async (app) => {
    ...
    if (Flag.NEOCODE_CONFIG) {
      result = mergeDeep(result, await load(Flag.NEOCODE_CONFIG))
    ...
    const markdownAgents = [
      ...
      ...(await Filesystem.globUp(".neocode/agent/*.md", app.path.cwd, app.path.root)),
    ]
  ...
  export const global = lazy(async () => {
    let result = pipe(
      ...
      mergeDeep(await load(path.join(Global.Path.config, "neocode.json"))),
    )
