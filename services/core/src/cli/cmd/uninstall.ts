...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall neocode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall NeoCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g neocode-ai",
      pnpm: "pnpm uninstall -g neocode-ai",
      bun: "bun remove -g neocode-ai",
      yarn: "yarn global remove neocode-ai",
      brew: "brew uninstall neocode",
      choco: "choco uninstall neocode",
      scoop: "scoop uninstall neocode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall neocode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using NeoCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# neocode") || content.includes(".neocode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".neocode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".neocode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".neocode"))
    ) {
      continue
    }
