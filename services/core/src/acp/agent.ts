...
import type { AssistantMessage, Event, NeocodeClient, SessionMessageResponse } from "@neocode-ai/pkg-sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: NeocodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `neocode auth login` in the terminal",
        name: "Login with neocode",
        id: "neocode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // NeoCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const neocodeProvider = providers.find((p) => p.id === "neocode")
    if (neocodeProvider) {
      if (neocodeProvider.models["big-pickle"]) {
        return { providerID: "neocode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(neocodeProvider.models))
      if (best) {
        return {
