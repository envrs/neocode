...
test("custom model inherits npm package from neomodels.vercel.app provider config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "neocode.json"),
...
test("custom model inherits api.url from neomodels.vercel.app provider", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "neocode.json"),
