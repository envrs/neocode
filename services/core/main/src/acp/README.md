...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for neocode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal neocode sessions
...
neocode acp --cwd /path/to/project
```
...
```bash
NEOCODE_ENABLE_QUESTION_TOOL=1 neocode acp
```
...
- **Terminal Integration**: Full terminal support via neocode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | neocode acp
```
...
ACP sessions map cleanly to neocode's internal session model:
