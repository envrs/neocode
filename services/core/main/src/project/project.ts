  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Bun.file(path.join(dotgit, "neocode"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.NEOCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(dotgit, "neocode"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.NEOCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
