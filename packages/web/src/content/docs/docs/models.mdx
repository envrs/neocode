...
You can configure providers in your neocode config under the `provider` section.

---

### Defaults

Most popular providers are preloaded by default. If you've added the credentials for a provider through `neocode auth login`, they'll be available when you start neocode.
...
```json title="neocode.json" {5,9-11}
{
...
When neocode starts up, it checks for the following:

1. The model list in the neocode config.
