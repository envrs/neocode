...
You can provide custom instructions to neocode by creating an `AGENTS.md` file. This is similar to `CLAUDE.md` or Cursor's rules. It contains instructions that will be included in the LLM's context to customize its behavior for your specific project.
...
So when neocode starts, it looks for:

1. **Local files** by traversing up from the current directory
2. **Global file** by checking `~/.config/neocode/AGENTS.md`

If you have both global and project-specific rules, neocode will combine them together.
