...
export function Code<T>(props: CodeProps<T>) {
  ...
  const clearHighlightFind = () => {
    ...
    if (!api) return
    api.delete("neocode-find")
    api.delete("neocode-find-current")
  }
  ...
  const setHighlights = (ranges: Range[], index: number) => {
    ...
    if (active) api.set("neocode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== index)
    if (rest.length > 0) api.set("neocode-find", new Highlight(...rest))
    return true
  }
