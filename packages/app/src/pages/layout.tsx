...
import { Avatar } from "@neocode-ai/ui/avatar"
...
import { InlineInput } from "@neocode-ai/ui/inline-input"
import { List, type ListRef } from "@neocode-ai/ui/list"
import { Tooltip, TooltipKeybind } from "@neocode-ai/ui/tooltip"
import { HoverCard } from "@neocode-ai/ui/hover-card"
import { MessageNav } from "@neocode-ai/ui/message-nav"
import { DropdownMenu } from "@neocode-ai/ui/dropdown-menu"
import { ContextMenu } from "@neocode-ai/ui/context-menu"
import { Collapsible } from "@neocode-ai/ui/collapsible"
import { DiffChanges } from "@neocode-ai/ui/diff-changes"
import { Spinner } from "@neocode-ai/ui/spinner"
import { Dialog } from "@neocode-ai/ui/dialog"
import { getFilename } from "@neocode-ai/util/path"
import { Session, type Message, type TextPart } from "@neocode-ai/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@neocode-ai/ui/toast"
import { useGlobalSDK } from "@/context/global-sdk"
import { useNotification } from "@/context/notification"
import { usePermission } from "@/context/permission"
import { Binary } from "@neocode-ai/util/binary"
import { retry } from "@neocode-ai/util/retry"
...
export default function Layout(props: ParentProps) {
  ...
  const deepLinkEvent = "neocode:deep-link"
  ...
  const drainDeepLinks = () => {
    const pending = window.__NEOCODE__?.deepLinks ?? []
    if (pending.length === 0) return
    if (window.__NEOCODE__) window.__NEOCODE__.deepLinks = []
  ...
  const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
    ...
    const neocode = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
    ...
    return (
      <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
        <div class="size-full rounded overflow-clip">
          <Avatar
            fallback={name()}
            src={props.project.id === neocode ? "https://neo.khulnasoft.com/favicon.svg" : props.project.icon?.override}
              ...
              <div class="overflow-y-auto max-h-72 h-full">
                <MessageNav
                  ...
                  onMessageSelect={(message) => {
                    if (!isActive()) {
                      sessionStorage.setItem("neocode.pendingMessage", `${props.session.id}|${message.id}`)
          ...
          <div class="shrink-0 w-full pt-3 pb-3 flex flex-col items-center gap-2">
            ...
            <Tooltip placement={sidebarProps.mobile ? "bottom" : "right"} value={language.t("sidebar.help")}>
              <IconButton
                ...
                variant="ghost"
                size="large"
                onClick={() => platform.openLink("https://neo.khulnasoft.com/desktop-feedback")}
