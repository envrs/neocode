...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("neocode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("neocode_lib=debug,neocode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("neocode_lib=info,neocode_desktop=info,sidecar=info")
        }
    });
