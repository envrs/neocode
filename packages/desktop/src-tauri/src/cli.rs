...
const CLI_INSTALL_DIR: &str = ".neocode/bin";
const CLI_BINARY_NAME: &str = "neocode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("neocode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "NEOCODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "NEOCODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("NEOCODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.neocode/bin/neocode\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://neo.khulnasoft.com/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "NEOCODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "NEOCODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "NEOCODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "NEOCODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "NEOCODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "NEOCODE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("NEOCODE_SERVER_USERNAME", "neocode".to_string()),
        ("NEOCODE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn neocode");
