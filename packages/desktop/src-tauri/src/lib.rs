...
const SETTINGS_STORE: &str = "neocode.settings.dat";
...
fn get_sidecar_port() -> u32 {
    option_env!("NEOCODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("NEOCODE_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, hostname: &str, port: u32, password: &str) -> CommandChild {
    ...
    .env("NEOCODE_SERVER_USERNAME", "neocode")
    .env("NEOCODE_SERVER_PASSWORD", password)
    .spawn()
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("neocode", Some(password));
    }
        ...
        .setup(move |app| {
            ...
            let window_builder = WebviewWindowBuilder::from_config(&app, config)
                ...
                .initialization_script(format!(
                    r#"
                      window.__NEOCODE__ ??= {{}};
                      window.__NEOCODE__.updaterEnabled = {updater_enabled};
                    "#
                ));
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            let _ = child.kill();
            break Err(format!(
                "Failed to spawn NeoCode Server. Logs:\n{}",
