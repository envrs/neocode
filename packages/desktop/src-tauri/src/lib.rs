...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %neocode_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn NeoCode Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
async fn setup_server_connection(app: AppHandle) -> ServerConnection {
    ...
    ServerConnection::CLI {
        url: local_url,
        username: Some("neocode".to_string()),
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = neocode_db_path() else {
        return true;
    };
...
fn neocode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("neocode").join("neocode.db"))
}
