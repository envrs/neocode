...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@neocode-ai/app"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NEOCODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://neo.khulnasoft.com/favicon-96x96-v3.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`neocode:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__NEOCODE__ ??= {}
            window.__NEOCODE__.serverPassword = data().password ?? undefined
