...
async function writeConfig(dir: string, config: object, name = "neocode.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "neocode.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "neocode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .neocode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const neocodeDir = path.join(dir, ".neocode")
      await fs.mkdir(neocodeDir, { recursive: true })
      const agentDir = path.join(neocodeDir, "agent")
...
test("loads agents from .neocode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const neocodeDir = path.join(dir, ".neocode")
      await fs.mkdir(neocodeDir, { recursive: true })

      const agentsDir = path.join(neocodeDir, "agents")
...
test("loads commands from .neocode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const neocodeDir = path.join(dir, ".neocode")
      await fs.mkdir(neocodeDir, { recursive: true })

      const commandDir = path.join(neocodeDir, "command")
...
test("loads commands from .neocode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const neocodeDir = path.join(dir, ".neocode")
      await fs.mkdir(neocodeDir, { recursive: true })

      const commandsDir = path.join(neocodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "neocode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .neocode config
      const projectDir = path.join(dir, "project")
      const neocodeDir = path.join(projectDir, ".neocode")
      await fs.mkdir(neocodeDir, { recursive: true })
      ...
      // Local .neocode config with different plugins
      await Bun.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .neocode config with some overlapping plugins
      await Bun.write(
        path.join(neocodeDir, "neocode.json"),
        JSON.stringify({
          $schema: "https://neo.khulnasoft.com/config.json",
...
// Note: preload.ts sets NEOCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .neocode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .neocode directory config enables it
      const neocodeDir = path.join(dir, ".neocode")
      await fs.mkdir(neocodeDir, { recursive: true })
      await Bun.write(
        path.join(neocodeDir, "neocode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/neocode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/neocode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-neocode@2.4.3")).toBe("oh-my-neocode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@neocode/plugin@2.0.0")).toBe("@neocode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-neocode@2.4.3", "file:///project/.neocode/plugin/oh-my-neocode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.neocode/plugin/oh-my-neocode.js")
  })
  ...
  test("local plugin directory overrides global neocode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const neocodeDir = path.join(projectDir, ".neocode")
        const pluginDir = path.join(neocodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("NEOCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NEOCODE_DISABLE_PROJECT_CONFIG"]
    process.env["NEOCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NEOCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NEOCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .neocode/ directories when flag is set", async () => {
    const originalEnv = process.env["NEOCODE_DISABLE_PROJECT_CONFIG"]
    process.env["NEOCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .neocode directory with a command
          const neocodeDir = path.join(dir, ".neocode", "command")
          await fs.mkdir(neocodeDir, { recursive: true })
          await Bun.write(path.join(neocodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .neocode should NOT be in directories list
          const hasProjectNeocode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectNeocode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NEOCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NEOCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NEOCODE_CONFIG_DIR"]
      process.env["NEOCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NEOCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NEOCODE_CONFIG_DIR"]
      } else {
        process.env["NEOCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NEOCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NEOCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NEOCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NEOCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NEOCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from NEOCODE_CONFIG_DIR, not project
