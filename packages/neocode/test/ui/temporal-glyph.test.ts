...
test("TemporalGlyphEngine - Single point trajectory", () => {
  ...
  const trajectory = TemporalGlyphEngine.generateTrajectory([{ state, timestamp: Date.now() }])
...
test("TemporalGlyphEngine - Performance metric calculation", () => {
  ...
  const highPerfTrajectory = TemporalGlyphEngine.generateTrajectory([{ state: highPerfState, timestamp: Date.now() }])

  const lowPerfTrajectory = TemporalGlyphEngine.generateTrajectory([{ state: lowPerfState, timestamp: Date.now() }])
...
test("TemporalGlyphEngine - Reliability metric calculation", () => {
  ...
  const prodTrajectory = TemporalGlyphEngine.generateTrajectory([{ state: prodState, timestamp: Date.now() }])

  const devTrajectory = TemporalGlyphEngine.generateTrajectory([{ state: devState, timestamp: Date.now() }])

  expect(prodTrajectory.points[0].metrics.reliability).toBeGreaterThan(devTrajectory.points[0].metrics.reliability)
})
...
test("TemporalGlyphEngine - Complexity metric calculation", () => {
  ...
  const simpleTrajectory = TemporalGlyphEngine.generateTrajectory([{ state: simpleState, timestamp: Date.now() }])

  const complexTrajectory = TemporalGlyphEngine.generateTrajectory([{ state: complexState, timestamp: Date.now() }])

  expect(complexTrajectory.points[0].metrics.complexity).toBeGreaterThan(simpleTrajectory.points[0].metrics.complexity)
})
...
test("TemporalGlyphEngine - Convergence calculation", () => {
  ...
  expect(stableTrajectory.trajectory.convergence).toBeGreaterThan(volatileTrajectory.trajectory.convergence)
})
