...
test("discovers skills from .neocode/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".neocode", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".neocode", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".neocode", "skill", "dir-skill")
...
test("discovers multiple skills from .neocode/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".neocode", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".neocode", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".neocode", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.NEOCODE_TEST_HOME
  process.env.NEOCODE_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.NEOCODE_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const neocodeSkillDir = path.join(dir, ".neocode", "skill", "agent-skill")
      const neocodeSkillsDir = path.join(dir, ".neocode", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(neocodeSkillDir, "SKILL.md"),
        `---
name: neocode-skill
description: A skill in the .neocode/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(neocodeSkillsDir, "SKILL.md"),
        `---
name: neocode-skill
description: A skill in the .neocode/skills directory.
---
