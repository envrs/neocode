...
export class TemporalGlyphEngine {
  ...
  static generateTrajectory(states: { state: SystemState; timestamp: number }[]): LearningTrajectory {
  ...
  private static calculateSystemMetrics(state: SystemState, glyph: string[]): TemporalGlyphPoint["metrics"] {
  ...
  private static generateInsights(
    ...
    const optimalPoint = points.reduce((best, current) => {
      ...
      const bestScore =
        best.metrics.performance + best.metrics.reliability + best.metrics.stability - best.metrics.complexity
  ...
  static renderTemporalEvolution(trajectory: LearningTrajectory): string[] {
    ...
    if (trajectory.insights.warningPoints.length > 0) {
      ...
      trajectory.insights.warningPoints.forEach((point) => {
        output.push(
          `  ${new Date(point.timestamp).toLocaleString()}: Reliability dropped to ${(point.metrics.reliability * 100).toFixed(1)}%`,
        )
      })
    }
    ...
    trajectory.points.forEach((point, index) => {
      ...
      output.push(`\n${index + 1}. ${date} ${time}`)
      output.push(
        `   Performance: ${(point.metrics.performance * 100).toFixed(1)}% | Reliability: ${(point.metrics.reliability * 100).toFixed(1)}% | Stability: ${(point.metrics.stability * 100).toFixed(1)}%`,
      )
