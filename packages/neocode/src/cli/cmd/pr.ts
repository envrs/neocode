...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run neocode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for neocode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found neocode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`neocode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting neocode...")
        UI.println()

        // Launch neocode TUI with session ID if available
        const { spawn } = await import("child_process")
        const neocodeArgs = sessionId ? ["-s", sessionId] : []
        const neocodeProcess = spawn("neocode", neocodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          neocodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`neocode exited with code ${code}`))
          })
          neocodeProcess.on("error", reject)
        })
      },
    })
  },
