...
import type { Hooks } from "@neocode-ai/plugin"
 ...
 * already present in neomodels.vercel.app, respecting enabled/disabled provider lists.
...
export const AuthLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs.positional("url", {
      describe: "neocode auth provider",
      type: "string",
    }),
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          const wellknown = await fetch(`${args.url}/.well-known/neocode`).then((x) => x.json() as any)
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in neocode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via neocode.json options (profile, region, endpoint) or\n" +
