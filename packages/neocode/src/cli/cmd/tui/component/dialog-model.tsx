...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "neocode" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "neocode" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "neocode" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "neocode" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "neocode" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
