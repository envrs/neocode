...
function main() {
  ...
  const deploymentDiff = GlyphDiffEngine.computeDiff(
    scenarios[0].fromState,
    scenarios[0].toState
  )
  ...
  const diffs = scenarios.map(scenario => 
    GlyphDiffEngine.computeDiff(scenario.fromState, scenario.toState)
  )
  
  console.log('\n🎯 Impact Distribution:')
  const impactCounts = diffs.reduce((acc, diff) => {
  ...
  console.log('\n⚠️  Risk Distribution:')
  const riskCounts = diffs.reduce((acc, diff) => {
  ...
  diffs.forEach((diff, index) => {
    ...
    console.log(`    Structural: ${(structural * 100).toFixed(1)}% | Semantic: ${semanticCount} changes | Confidence: ${(confidence * 100).toFixed(1)}%`)
  })
