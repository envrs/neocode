...
import { createNeocode, type ToolPart } from "@neocode-ai/pkg-sdk"
...
console.log("🚀 Starting neocode server...")
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new neocode session...")
    ...
    console.log("✅ Created neocode session:", createResult.data.id)
  ...
  console.log("📝 Sending to neocode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Neocode response:", JSON.stringify(result, null, 2))
