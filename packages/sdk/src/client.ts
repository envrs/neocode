...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['NEOCODE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['NEOCODE_LOG'] or 'warn' if it isn't set.
   */
...
export class Neocode {
  ...
  constructor({ baseURL = readEnv('NEOCODE_BASE_URL'), ...opts }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Neocode.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('NEOCODE_LOG'), "process.env['NEOCODE_LOG']", this) ??
      defaultLogLevel;
  ...
  static NeocodeError = Errors.NeocodeError;
