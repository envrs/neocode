...
func renderText(
	app *app.App,
	message neocode.MessageUnion,
	text string,
	author string,
	showToolDetails bool,
	width int,
	extra string,
	toolCalls ...neocode.ToolPart,
) string {
	t := theme.CurrentTheme()
	...
	if !showToolDetails && toolCalls != nil && len(toolCalls) > 0 {
		...
		for _, toolCall := range toolCalls {
			...
			style := styles.NewStyle()
			if toolCall.State.Status == neocode.ToolPartStateStatusError {
...
func renderToolDetails(
	...
	if toolCall.State.Status == neocode.ToolPartStateStatusPending {
		title := renderToolTitle(toolCall, width)
...
func renderToolName(name string) string {
	...
	default:
		normalizedName := name
		if after, ok := strings.CutPrefix(name, "neocode_"); ok {
...
func getTodoTitle(toolCall neocode.ToolPart) string {
	if toolCall.State.Status == neocode.ToolPartStateStatusCompleted {
