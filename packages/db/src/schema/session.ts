...
type PartData = Record<string, any>
type InfoData = Record<string, any>
...
export const SessionTable = sqliteTable(
  "session",
  {
    ...
    summary_diffs: text({ mode: "json" }).$type<DBTypes.FileDiff[]>(),
    revert: text({ mode: "json" }).$type<{ messageID: string; partID?: string; snapshot?: string; diff?: string }>(),
    permission: text({ mode: "json" }).$type<DBTypes.Ruleset>(),
    ...Timestamps,
...
export const PermissionTable = sqliteTable("permission", {
  project_id: text()
    ...
    .references(() => ProjectTable.id, { onDelete: "cascade" }),
  ...Timestamps,
  data: text({ mode: "json" }).notNull().$type<DBTypes.Ruleset>(),
})
